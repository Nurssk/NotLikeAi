...
const CrowdCanvas = ({ src, rows = 15, cols = 7 }: CrowdCanvasProps) => {
  ...
  useEffect(() => {
    ...
    // Responsive sprite scale — fluid with viewport width, clamped so people
    // stay clearly visible (never tiny) on big screens and never overwhelming
    // on mobile. ~1.2 on large desktops → ~0.58 on small phones.
    const getSpriteScale = () => {
      const vw = window.innerWidth;
      const scale = (vw / 1440) * 0.72;
      return Math.min(0.86, Math.max(0.42, scale));
    };
    ...
    const normalWalk = ({ peep, props }: { peep: any; props: any }) => {
      ...
      // Slower, more controlled crowd movement (was randomRange(0.5, 1.5))
      tl.timeScale(0.75);
      tl.to(
        peep,
        {
    ...
    const createPeep = ({
      ...
      const peep: Peep = {
        ...
        setRect: (rect: number[]) => {
          peep.rect = rect;
          // Keep the original sprite-sheet rect; derive displayed size from it
          peep.sourceWidth = rect[2];
          peep.sourceHeight = rect[3];
    ...
    const resize = () => {
      ...
      // Re-scale every peep to the current viewport so walking paths,
      // anchorY, and crop all use the new displayed dimensions.
      const spriteScale = getSpriteScale();
      allPeeps.forEach((peep) => {
        peep.setScale(spriteScale);
      });
