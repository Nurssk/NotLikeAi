...
type ResolvedGradualBlurProps = Omit<
  Required<GradualBlurProps>,
  "children" | "onAnimationComplete" | "hoverIntensity" | "width"
> & {
  width?: string;
  hoverIntensity?: number;
  onAnimationComplete?: () => void;
...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calculate = () => {
      ...
      const configKey = capitalize(key);
      let nextValue = config[key];

      if (viewportWidth <= 480 && config[`mobile${configKey}` as keyof GradualBlurProps]) {
        nextValue = config[`mobile${configKey}` as keyof GradualBlurProps] as string;
      } else if (viewportWidth <= 768 && config[`tablet${configKey}` as keyof GradualBlurProps]) {
        nextValue = config[`tablet${configKey}` as keyof GradualBlurProps] as string;
      } else if (viewportWidth <= 1024 && config[`desktop${configKey}` as keyof GradualBlurProps]) {
        nextValue = config[`desktop${configKey}` as keyof GradualBlurProps] as string;
      }
...
const GradualBlur: React.FC<GradualBlurProps> = (props) => {
  ...
  const config = useMemo(() => {
    ...
    return mergeConfigs(DEFAULT_CONFIG, presetConfig, props) as ResolvedGradualBlurProps;
  }, [props]);
