...
import { r as readServerEnv, n as normalizeEmail, i as isValidEmail, f as emailToCreditDocId } from './server-env_D1vftNvA.mjs';
...
const EXTENSION_AUTH_CODES_COLLECTION = "extensionAuthCodes";
const extensionAuthCorsHeaders = {
...
const extensionAuthJsonResponse = (body, init = {}) => new Response(JSON.stringify(body), {
  ...init,
  headers: {
...
const base64UrlEncode = (value) => Buffer.from(value).toString("base64").replace(/\+/g, "-").replace(/\//g, "_").replace(/=+$/g, "");
...
const hmacBase64Url = (value) => base64UrlEncode(createHmac("sha256", getExtensionAuthSecret()).update(value).digest());
const timingSafeEqualString = (left, right) => {
  const leftBuffer = Buffer.from(left);
  const rightBuffer = Buffer.from(right);
  return leftBuffer.length === rightBuffer.length && timingSafeEqual(leftBuffer, rightBuffer);
};
const normalizeExtensionCode = (code) => code.trim().toUpperCase().replace(/[^A-Z0-9]/g, "");
const formatExtensionCode = (code) => code;
...
const verifyWebsiteFirebaseRequest = async (request) => {
...
const createExtensionAuthCode = async (request) => {
  ...
  return extensionAuthJsonResponse({
    ...
    expiresInSeconds: Math.floor(CODE_TTL_MS / 1e3),
    email: verified.normalizedEmail
  });
};
const createExtensionSessionToken = (payload) => {
...
const verifyExtensionSessionToken = (token) => {
...
const exchangeExtensionAuthCode = async (request) => {
  ...
  const db = adminDb();
  const snapshot = await db.collection(EXTENSION_AUTH_CODES_COLLECTION).where("email", "==", email).where("code", "==", code).limit(1).get();
  if (snapshot.empty) {
...
export { extensionAuthJsonResponse as a, exchangeExtensionAuthCode as b, createExtensionAuthCode as c, adminDb as d, extensionAuthCorsHeaders as e, adminAuth as f, verifyExtensionSessionToken as v };
